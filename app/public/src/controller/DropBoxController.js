class DropBoxController {
    ...
    connectFirebase(){
        ...
        var firebaseConfig = {
            apiKey: "AIzaSyB2o-5-f7vJaVSVkCUEjqYySU8P34aJavM",
            authDomain: "dropbox-clone-c763a.firebaseapp.com",
            databaseURL: "https://dropbox-clone-c763a-default-rtdb.firebaseio.com",
            projectId: "dropbox-clone-c763a",
            storageBucket: "dropbox-clone-c763a.appspot.com",
            messagingSenderId: "15292751072",
            appId: "1:15292751072:web:a7361293b69b0a83bcc485",
            measurementId: "G-BN60296LSC"
        };
        // Initialize Firebase
    ...
    initEvents(){
            ...
            this.removeTask().then(responses=>{
                responses.forEach(response=>{
                    if(response.fields.key){
                        this.getFirebaseRef().child(response.fields.key).remove(); //apagando da database
                    }
                })
            }).catch(err=>{
                console.error(err);
        ...
        this.btnRename.addEventListener('click',e=>{
            ...
            if (name){
                file.name = name;
                //child vai procurar a chave do arquivo 
                //set vai trocar a informação do banco de dados para a que ta dentro do set()
    ...
    ajax(url, method = 'GET',formData=new FormData(),onprogress=function(){},onloadstart=function(){}){
        return new Promise ((resolve,reject)=>{
            let ajax = new XMLHttpRequest();    // classe que envia requisições http
            ajax.open(method,url);              // open() inicializa o pedido // method (GET, POST) and route ( /upload , /file etc )
            ajax.onload = event => {            // onload é chamado quando a operação é concluída com êxito
                try{
                    resolve(JSON.parse(ajax.responseText));  // responseText retorna o texto recebido de um servidor após o envio de uma solicitação.
                } catch (e) {
                    reject(e);
                }
            };
            ...
            ajax.upload.onprogress = onprogress; // .upload é objeto que pode ser observado para monitorar o andamento de um upload // onprogress é Entregue periodicamente para indicar o progresso feito até agora.
            onloadstart();                       //  invocado exatamente uma vez quando a operação começa.
            ajax.send(formData);                 // envia a solicitação //envia o arquivo que está nos campos do formulário formdata eu acho
        });
    }
    ...
    uploadTask(files){
        ...
        [...files].forEach(file=>{
            promises.push(new Promise((resolve, reject)=>{
                let fileRef = firebase.storage().ref(this.currentFolder.join('/')).child(file.name); // ref() -> a referencia do  caminho, child(file.name) -> dentro da referencia cria um arquivo com esse nome 
                let task = fileRef.put(file);                                                        // faz o upload do arquivo e retorna eventos para manipularmos o upload
                this.startUploadTime = Date.now();
                task.on('state_changed', snapshot => {                                               // task.on('state_changed', progress, error, resolve);
                    this.uploadProgress({
                    ...
                    fileRef.getMetadata().then(metadata=>{
                        ...
                        fileRef.getDownloadURL().then(url=>{
                            let metadataURL = Object.assign(metadata,{url:url});
                            resolve(metadataURL);
                        });
                        // solução antiga
                        //resolve(metadata);
        ...
        return Promise.all(promises); //recebe o array de promises
    }
    ...
    formatTimeToHuman(duration){
        ...
        if(seconds>1){
            ...
            return `${seconds} segundo restante`;
        }
    ...
    readFiles(){
        ...
        // on() vai ficar ouvindo o banco 
        // snapshot é uma foto do momento de como está o banco
    ...
    initEventsLi(li){
        ...
        li.addEventListener('click',e=>{
            //nao sei oq eh this.onselectionchange = new Event('selectionchange');
            ...
            if(e.shiftKey){
                ...
                if(this.lastLi){
                    lis.forEach((el,index)=>{
                        if(this.lastLi===el) indexStart = index;
